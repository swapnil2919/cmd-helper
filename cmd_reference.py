...
LEARN_PATHS = {
    "Terminal Basics": {
        "desc": "Navigation, files, pipes — the foundation everything else builds on",
        "lessons": [
            {
                "title": "Navigation",
                "concept": [
                    "The terminal puts you in exactly one directory at all times.",
                    "pwd shows where you are. ls shows what's here. cd moves you.",
                    "Master these three and you can find anything on any system.",
                ],
                "commands": [
                    ("pwd",        "Where am I right now?"),
                    ("ls",         "What's in the current directory?"),
                    ("ls -la",     "Full details + hidden files (starts with .)"),
                    ("cd <dir>",   "Enter a directory"),
                    ("cd ..",      "Go up one level"),
                    ("cd ~",       "Jump to home directory"),
                    ("cd -",       "Return to previous directory"),
                ],
                "tip": "cd - is like a browser back button — really useful for switching between two dirs.",
                "challenge": "cd /tmp && ls -la && cd -  (should take you back to where you were)",
            },
            {
                "title": "Files & Directories",
                "concept": [
                    "Everything is a file in Unix. You can create, copy, move, delete.",
                    "mkdir creates directories. touch creates empty files.",
                    "rm is permanent — there is no recycle bin from the terminal.",
                ],
                "commands": [
                    ("mkdir <dir>",      "Create a directory"),
                    ("mkdir -p a/b/c",   "Create nested directories at once"),
                    ("touch <file>",     "Create an empty file"),
                    ("cp src dst",       "Copy file  (cp -r for directories)"),
                    ("mv old new",       "Move or rename file/directory"),
                    ("rm <file>",        "Delete file — PERMANENT, no undo"),
                    ("rm -rf <dir>",     "Delete directory and everything inside"),
                ],
                "tip": "Always double-check rm -rf paths. There is no ctrl+Z in the terminal.",
                "challenge": "mkdir practice && cd practice && touch a.txt b.txt && ls && cd .. && rm -rf practice",
            },
            {
                "title": "Viewing Files",
                "concept": [
                    "Read file contents without opening a GUI editor.",
                    "cat dumps everything. less lets you scroll (q to quit).",
                    "grep searches inside files — indispensable for logs and code.",
                ],
                "commands": [
                    ("cat <file>",         "Print entire file"),
                    ("less <file>",        "Scroll file — q quit, / search, n next match"),
                    ("head -n 20 <file>",  "First 20 lines"),
                    ("tail -n 20 <file>",  "Last 20 lines"),
                    ("tail -f <file>",     "Follow file live — great for logs"),
                    ("grep 'text' <file>", "Search for text in file"),
                    ("grep -r 'text' .",   "Recursive search across all files here"),
                ],
                "tip": "tail -f on a log file is real-time monitoring — run it while your app is running.",
                "challenge": "grep -r 'def ' . to find all Python function definitions in current dir",
            },
            {
                "title": "Pipes & Redirection",
                "concept": [
                    "Pipes (|) chain commands — output of one becomes input of the next.",
                    "Redirection (> and >>) saves output to files.",
                    "This is the shell's superpower — build complex workflows from simple tools.",
                ],
                "commands": [
                    ("cmd1 | cmd2",       "Pipe: output of cmd1 feeds into cmd2"),
                    ("cmd > file",        "Write output to file (overwrites)"),
                    ("cmd >> file",       "Append output to file"),
                    ("ls | grep .txt",    "List only .txt files"),
                    ("ps aux | grep vim", "Check if vim is running"),
                    ("cat f | wc -l",     "Count lines in file"),
                    ("cmd 2>&1 | other",  "Include stderr in the pipe"),
                ],
                "tip": "ps aux | grep <name> is how pros check if a process is running and get its PID.",
                "challenge": "ls /usr/bin | grep git — see all git-related binaries installed on your system",
            },
        ],
    },
    "Git & Version Control": {
        "desc": "Track code, collaborate with teams, and never lose work again",
        "lessons": [
            {
                "title": "The Core Workflow",
                "concept": [
                    "Git takes snapshots of your code called commits.",
                    "Files go: Untracked → Staged (git add) → Committed (git commit).",
                    "git status is your compass — run it constantly.",
                ],
                "commands": [
                    ("git init",            "Start tracking a directory"),
                    ("git clone <url>",     "Download a repository"),
                    ("git status",          "See what changed — your most-used command"),
                    ("git add <file>",      "Stage a specific file"),
                    ("git add .",           "Stage all changed files"),
                    ("git commit -m 'msg'", "Save staged snapshot with a message"),
                    ("git log --oneline",   "Compact commit history"),
                ],
                "tip": "Commit early, commit often. Small commits are easier to debug and revert.",
                "challenge": "git init test && cd test && touch README.md && git add . && git commit -m 'init'",
            },
            {
                "title": "Branches",
                "concept": [
                    "Branches let you work on features without touching main code.",
                    "Create a branch per feature or fix. Merge when done.",
                    "Every professional team works this way.",
                ],
                "commands": [
                    ...
                    ("git diff main..<branch>", "See what differs from main"),
                ],
                "tip": "Name branches descriptively: feature/user-login, fix/crash-on-save.",
                "challenge": "git checkout -b feature/test && touch x.txt && git add . && git commit -m 'test' && git checkout main",
            },
            {
                "title": "Remotes & Collaboration",
                "concept": [
                    "Remotes are your repo hosted on a server (GitHub/GitLab).",
                    "push sends commits up, pull brings others' commits down.",
                ...
                "commands": [
                    ...
                    ("git pull --rebase",          "Pull, replay your commits on top"),
                    ("git fetch --all",            "Download all remote changes"),
                    ("git remote add origin <url>","Connect a remote"),
                ],
                "tip": "git pull --rebase keeps a cleaner linear history than plain git pull.",
                "challenge": "git remote -v on any existing repo, or git clone any public GitHub repo.",
            },
            {
                "title": "Undo & Recovery",
                "concept": [
                    "Git is time travel — you can almost always recover from mistakes.",
                    "Stash saves WIP. Reset undoes commits. Reflog is the ultimate safety net.",
                    "Never panic about git mistakes — there is almost always a way back.",
                ],
                "commands": [
                    ...
                    ("git revert <hash>",     "Create an undo commit (safe for shared branches)"),
                    ("git diff --staged",     "Review what you are about to commit"),
                    ("git reflog",            "Full history — ultimate recovery tool"),
                ],
                "tip": "git reflog can recover commits even after reset --hard. Learn it before you need it.",
                "challenge": "Make a change, git stash it, check git status (clean?), then git stash pop",
            },
        ],
    },
    "Docker & Containers": {
        "desc": "Package apps to run anywhere — the modern way to ship software",
        "lessons": [
            {
                "title": "Images & Containers",
                "concept": [
                    "Image = blueprint (like a class). Container = running instance (like an object).",
                    "Pull images from Docker Hub. Run them as containers.",
                    "Containers are isolated — they cannot mess with your system.",
                ],
                "commands": [
                    ("docker ps",                  "List running containers"),
                ...
                "tip": "docker run --rm auto-deletes when done — great for one-off commands.",
                "challenge": "docker run --rm hello-world",
            },
            {
                "title": "Ports, Volumes & Env",
                "concept": [
                    "Containers are isolated — you must explicitly map ports and files.",
                    "-p maps host:container port. -v mounts a directory from host.",
                    "-e passes environment variables (API keys, DB passwords, etc.).",
                ],
                "commands": [
                    ("docker run -p 8080:80 nginx",      "Nginx accessible on localhost:8080"),
                    ("docker run -d nginx",              "Run in background (detached)"),
                    ("docker run -v ./data:/data img",   "Mount ./data into container"),
                    ("docker run -e DB_URL=... img",     "Pass env variable"),
                    ("docker logs <id>",                 "View container output"),
                    ("docker logs -f <id>",              "Follow logs live"),
                    ("docker exec -it <id> bash",        "Shell into running container"),
                ],
                "tip": "docker exec is how you debug a running container — like SSH but for containers.",
                "challenge": "docker run -d -p 8080:80 nginx && curl localhost:8080 | head -5",
            },
            {
                "title": "Docker Compose",
                "concept": [
                    "Compose runs multi-container apps from a docker-compose.yml file.",
                    "Define your app, database, and cache together. One command starts everything.",
                    "This is how real apps are developed locally.",
                ],
                "commands": [
                ...
                "tip": "docker compose up -d && docker compose logs -f is your standard daily startup.",
                "challenge": "Write a docker-compose.yml with one nginx service and run docker compose up -d",
            },
        ],
    },
    "SSH & Remote Work": {
        "desc": "Connect to and manage remote servers — essential for every developer",
        "lessons": [
            {
                "title": "SSH Basics",
                "concept": [
                    "SSH gives you a terminal on a remote machine over an encrypted connection.",
                    "Most cloud servers (AWS, DigitalOcean, VPS) are managed 100% via SSH.",
                    "Once connected it is just like your local terminal.",
                ],
                "commands": [
                    ("ssh user@host",           "Connect to server"),
                    ("ssh user@host -p 2222",   "SSH on custom port"),
                    ("ssh -i key.pem user@host","Connect with private key"),
                    ("exit",                    "Disconnect (or Ctrl+D)"),
                    ("ssh user@host 'ls -la'",  "Run single command remotely"),
                    ("ssh -v user@host",        "Verbose output for debugging"),
                    ("~.",                      "Force disconnect if SSH hangs"),
                ],
                "tip": "ssh user@host 'cmd' runs one command and returns — perfect for scripts.",
                "challenge": "ssh localhost (you may need to enable SSH on your system first)",
            },
            {
                "title": "SSH Keys",
                "concept": [
                    "SSH keys are more secure and convenient than passwords.",
                    "You have a private key (stays on your machine) and a public key (goes on servers).",
                    "GitHub and GitLab auth also uses SSH keys.",
                ],
                "commands": [
                    ...
                    ("chmod 600 ~/.ssh/id_ed25519", "Fix permissions if SSH rejects key"),
                    ("cat ~/.ssh/config",           "View SSH shortcuts config"),
                ],
                "tip": "Add a Host block in ~/.ssh/config to type 'ssh myserver' instead of the full address.",
                "challenge": "ssh-keygen -t ed25519 -f ~/.ssh/test_key -N '' && cat ~/.ssh/test_key.pub",
            },
            {
                "title": "Copying Files",
                "concept": [
                    "scp copies files over SSH. rsync syncs directories and resumes transfers.",
                    "rsync only transfers changed parts — much faster for large files.",
                    "Both use SSH keys automatically.",
                ],
                "commands": [
                    ("scp file user@host:/path/",       "Upload file to remote"),
                    ("scp user@host:/path/file .",       "Download file from remote"),
                    ("scp -r dir/ user@host:/path/",     "Upload directory"),
                    ("rsync -av src/ dst/",              "Sync local directories"),
                    ("rsync -av src/ user@host:/dst/",   "Sync to remote server"),
                    ("rsync -av --delete src/ dst/",     "Sync and delete extras at dst"),
                    ("rsync --dry-run -av src/ dst/",    "Preview changes without transferring"),
                ],
                "tip": "rsync --dry-run shows exactly what WOULD be copied. Always run it first for large syncs.",
                "challenge": "rsync -av --dry-run /etc/ /tmp/etc-preview/",
            },
        ],
    },
    "System Monitoring": {
        "desc": "Know what your machine is doing — debug slowness and resource issues",
        "lessons": [
            {
                "title": "Watching Processes",
                "concept": [
                    "Every running program is a process with a unique PID.",
                    "ps gives a snapshot. top/htop shows live updates.",
                    "Knowing what is running helps you debug and free resources.",
                ],
                "commands": [
                    ("ps aux",             "All running processes"),
                    ("ps aux | grep name", "Find a specific process"),
                    ("top",                "Live monitor (press q to quit)"),
                    ("htop",               "Better live monitor (install: apt/brew install htop)"),
                    ("pgrep <name>",       "Get PID(s) by process name"),
                    ("kill <PID>",         "Send stop signal"),
                    ("kill -9 <PID>",      "Force kill — last resort"),
                ],
                "tip": "Try plain kill first. kill -9 is brute force and skips graceful shutdown.",
                "challenge": "ps aux | grep -v grep | head -10 — top 10 running processes",
            },
            {
                "title": "Disk & Memory",
                "concept": [
                    "df shows disk space per filesystem. du shows per-directory usage.",
                    "free shows RAM and swap. ss shows open network ports.",
                    "These are the first commands you run when a server is slow.",
                ],
                "commands": [
                    ("df -h",                      "Disk space of all filesystems"),
                    ("df -h .",                     "Disk space of current filesystem"),
                    ("du -sh <dir>",                "How big is this directory?"),
                    ("du -ah | sort -rh | head -20","Top 20 biggest files/dirs"),
                    ("free -h",                     "RAM and swap usage"),
                    ("uptime",                      "System load average"),
                    ("ss -tuln",                    "All listening network ports"),
                ],
                "tip": "If disk is full: du -ah / | sort -rh | head -20 to find what is eating space.",
                "challenge": "df -h && echo '---' && free -h",
            },
        ],
    },
}
